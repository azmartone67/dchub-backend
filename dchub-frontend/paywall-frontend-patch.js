...
(function () {
  ...
  // -----------------------------------------------------------------------
  window.DCHub = window.DCHub || {};
  window.DCHub.fetch = dchubFetch;
  window.DCHub.getSession = function () {
    return Object.assign({}, currentSession);
  };
  window.DCHub.verifyNow = function (cb) {
    verifySession(cb);
  };
  window.DCHub.tierGte = tierGte;
  window.DCHub.upgradeUrl = UPGRADE_URL;
  window.DCHub.checkoutUrl = STRIPE_CHECKOUT;

  // Freeze the public API to prevent tampering
  try {
    Object.freeze(window.DCHub);
  } catch (_) {}
  ...
  function init() {
    ...
    // Re-apply visibility when DOM changes (SPA navigation)
