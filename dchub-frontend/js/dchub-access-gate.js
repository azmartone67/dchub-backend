...
(function () {
  ...
  function syncSessionFromBackend() {
    ...
    var token = localStorage.getItem('dchub_token') || getCookie('dchub_token');
    if (!token) return Promise.resolve();
    // Cross-subdomain: token arrived via cookie — persist to localStorage so subsequent checks work
    if (!localStorage.getItem('dchub_token')) {
      localStorage.setItem('dchub_token', token);
    }
    // v7.9.10 FIX: Tolerate transient 5xx from Railway. Keep cached session
    // and don't spam the console with warnings every page load. Only retry
    // after a short backoff — avoids the "503 → Session sync failed" spam
    // that shows up on testimonials/gdci/etc.
    _tierSyncPromise = fetch(API_BASE + '/api/auth/me', {
      headers: { 'Authorization': 'Bearer ' + token },
      mode: 'cors'
    }).then(function (r) {
      if (r.status === 401 || r.status === 403) {
        // real auth failure — token invalid; clear it
        throw new Error('auth_invalid:' + r.status);
      }
      if (!r.ok) {
        // 5xx / transient — keep existing cached session, schedule a retry
        var TRANSIENT = r.status >= 500 || r.status === 429 || r.status === 0;
        if (TRANSIENT) {
          _tierSynced = true; // don't thrash this pageload
          setTimeout(function () { _tierSynced = false; _tierSyncPromise = null; }, 60 * 1000);
          return null; // swallow silently
        }
        throw new Error('auth/me failed: ' + r.status);
      }
      return r.json();
    }).then(function (data) {
      _tierSynced = true;
      if (!data) return; // swallowed transient above
      ...
      // Only log real auth failures; transient 5xx already swallowed above.
      if (/^auth_invalid:/.test(e.message)) {
        console.warn('[DC Hub] Token invalid — clearing cached session');
        try {
  ...
  if (localStorage.getItem('dchub_token') || getCookie('dchub_token')) {
  ...
  function isLoggedIn() {
    ...
    var token = localStorage.getItem('dchub_token') || getCookie('dchub_token');
    return !!token && (!!localStorage.getItem('dchub_user') || !!localStorage.getItem('dchub_session'));
  }
    ...
    if (getUserTier() === 'anonymous') {
      ...
      setTimeout(function () {
        ...
        wall.innerHTML = '<div style="text-align:center;max-width:420px;padding:24px">' +
          ...
          '<p style="color:rgba(255,255,255,0.6);font-size:0.95rem;margin:0 0 8px">20,000+ facilities across 140+ countries</p>' +
