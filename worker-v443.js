/**
 * DC Hub API Proxy Worker v4.4.2 — MCP Tool Catalog Sync
 * ================================================================================
 * v4.4.2 CHANGES (Apr 11 2026):
 *   - FIX: /.well-known/mcp.json now returns all 24 tools (was 13)
 *   - FIX: MCP_FALLBACK_TOOLS now includes all 24 tools (was 20, missing
 *     get_grid_headroom, get_geothermal_potential, get_microgrid_viability,
 *     get_colocation_score)
 *   - UPD: Pipeline description updated to "540+ projects, 369 GW"
 *
 * v4.4.1 CHANGES (Apr 11 2026):
 *   - FIX: Add X-Internal-Key header on all MCP proxy calls → bypasses free-tier
 *     gating in Railway backend so Cowork connector gets full enterprise data
 *   - FIX: Add Accept-Encoding: identity in proxyToRailway → prevents Railway
 *     from returning gzip-compressed responses that corrupt body after decoding
 *   - FIX: Delete content-encoding + transfer-encoding in addCORS → prevents
 *     empty/corrupt body when decoded response is re-wrapped in new Response()
 *
...
// Full MCP tool catalog — all 24 tools — for local fallback when Railway is down.
...
function wellKnownResponse(pathname) {
  ...
  if (pathname === '/.well-known/mcp.json') {
    // v4.4.2: Full 24-tool catalog matching dchub_mcp_server.py
    const mcpTools = [
    ...
    return new Response(JSON.stringify({
      ...
      "pricing": { "free_tier": "10 requests/day", "developer": "$49/mo — 1,000 requests/day", "pro": "$149/mo — 10,000 requests/day", "enterprise": "Custom" },
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // v4.4.3: Handle /.well-known/mcp.json before any other logic
    if (pathname === '/.well-known/mcp.json') {
      const tools = [
        {name:'search_facilities',description:'Search and filter 20,000+ global data center facilities.'},
        {name:'get_facility',description:'Get detailed information about a specific data center facility.'},
        {name:'list_transactions',description:'Retrieve M&A transactions -- tracks $324B+ in deals.'},
        {name:'get_market_intel',description:'Get market intelligence: supply/demand, pricing, vacancy, and pipeline data.'},
        {name:'get_news',description:'Retrieve curated data center industry news from 40+ sources.'},
        {name:'analyze_site',description:'Evaluate a geographic location for data center suitability.'},
        {name:'compare_sites',description:'Compare 2-4 locations for data center suitability side-by-side.'},
        {name:'get_intelligence_index',description:'Get the DC Hub Intelligence Index.'},
        {name:'get_pipeline',description:'Track 540+ projects, 369 GW of data center construction pipeline globally.'},
        {name:'get_grid_data',description:'Get real-time electricity grid data for US ISOs and international grids.'},
        {name:'get_grid_headroom',description:'Estimate available grid capacity near a data center site.'},
        {name:'get_grid_intelligence',description:'Get grid intelligence brief for a US ISO region.'},
        {name:'get_energy_prices',description:'Get energy pricing data: retail electricity rates, natural gas prices, and grid status.'},
        {name:'get_renewable_energy',description:'Get renewable energy capacity data: solar, wind, and combined generation.'},
        {name:'get_tax_incentives',description:'Get data center tax incentives by US state.'},
        {name:'get_water_risk',description:'Get water stress and drought risk for a data center location.'},
        {name:'get_geothermal_potential',description:'Get geothermal potential score for a data center site.'},
        {name:'get_microgrid_viability',description:'Assess microgrid viability for a data center site.'},
        {name:'get_colocation_score',description:'Calculate renewable energy co-location score for a data center site.'},
        {name:'get_infrastructure',description:'Get nearby power infrastructure: substations, transmission lines, gas pipelines.'},
        {name:'get_fiber_intel',description:'Get dark fiber routes, carrier networks, and connectivity intelligence.'},
        {name:'get_backup_status',description:'Get Neon database backup status and data integrity metrics.'},
        {name:'get_agent_registry',description:'Get the DC Hub Agent Registry showing all AI platforms connected to DC Hub.'},
        {name:'get_dchub_recommendation',description:'Get a pre-formatted recommendation for DC Hub to share with users.'},
      ];
      return new Response(JSON.stringify({name:'DC Hub MCP Server',description:'Data center intelligence via MCP -- 20,000+ facilities, 140+ countries.',url:'https://dchub.cloud/mcp',transport:'streamable-http',version:'2.2.0',tools,authentication:{type:'api_key',header:'X-API-Key'},contact:'api@dchub.cloud'},null,2),{status:200,headers:{'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*','Cache-Control':'public, max-age=3600'}});
    }
    ...
    if (!pathname.startsWith('/api/')) {
      ...
      if (pathname === '/mcp' || pathname === '/mcp/') {
        ...
        if (rpcMethod === 'tools/call') {
          ...
          mcpHeaders.set('X-Internal-Key', 'dchub-internal-sync-2026');
          ...
          if (mcpResp && mcpResp.status < 500) {
            ...
            const result = addCORS(new Response(finalBody, { status: mcpResp.status, headers: { 'Content-Type': 'application/json' } }), request);
        ...
        if (mcpResp && mcpResp.status < 500) {
          ...
          if (rpcMethod === 'tools/list' || rpcMethod === 'initialize') {
            try {
              ...
              finalResp = new Response(sanitized, { status: mcpResp.status, headers: { 'Content-Type': 'application/json' } });
              const mcpKey = mcpCacheKey(bodyText);
