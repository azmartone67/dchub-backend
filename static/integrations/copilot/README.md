# DC Hub × Copilot — Integration Package

**Status:** Integration Ready
**Path:** Function Calling (Primary) / MCP streamable-http (Secondary)
**Verification Key:** `dchub_copilot_2026_verify` (Pro tier: 300 req/min, 100 results/query)
**AI Wars Score:** 93 — Highest of all platforms

---

## Quick Start

### Option A: Copilot Function Calling

```python
# Tool declarations for Copilot's function-calling system
# See function-calling.json for complete declarations of all 6 tools
tools = [
    {
        "type": "function",
        "function": {
...
```json
{
  "mcpServers": {
    "dchub": {
      ...
      "headers": {"X-API-Key": "dchub_copilot_2026_verify"}
    }
  }
}
```
...
```bash
curl -H "X-API-Key: dchub_copilot_2026_verify" \
