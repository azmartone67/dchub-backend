...
TOOL_TEASER = {
    ...
    "get_pocket_listings":    "teaser cards for off-market data center capacity DC Hub is onboarding — powered shells, available capacity and development sites that are not publicly marketed. Market, state and capacity are open to anyone; a listing's specs need an identified caller (a key with an email bound, or OAuth). The site and the provider's contact are shared only after the provider accepts a registration made with request_listing_intro.",
    "request_listing_intro":  "a registration for a pocket listing — or a standing requirement for first access to upcoming listings — recorded in DC Hub's hash-chained lead register with a public verification record. DC Hub emails a one-click confirmation, then sends the provider your human's company name and requirement; only if the provider accepts are the site details and both sides' contacts shared. Needs an identified caller and accept_terms=true once your human has agreed to the introduction terms.",
    "get_pocket_listing":     "one pocket listing's specs for an identified caller (call get_pocket_listings with slug) — the site and the provider's contact follow only after the provider accepts a registration.",
    # 2026-09-13: the Capacity Source names (see TOOL_TIER).
    "source_capacity":        "listing cards for DC Hub Capacity Source — powered land, powered shells and turnkey capacity to buy or lease, including sites that are not publicly marketed, each showing when it was last updated. Market, state and capacity are open to anyone; a listing's specs need an identified caller (a key with an email bound, or OAuth). The site and the provider's contact are shared only after the provider accepts a registration made with request_capacity_intro.",
    "request_capacity_intro": "a registration for a Capacity Source listing — or a standing requirement for first access as listings are onboarded — recorded in DC Hub's hash-chained lead register with a public verification record. DC Hub emails a one-click confirmation, then sends the provider your human's company name and requirement; only if the provider accepts are the site details and both sides' contacts shared. Needs an identified caller and accept_terms=true once your human has agreed to the introduction terms.",
    "accept_capacity_terms":  "the one-time acceptance of DC Hub's introduction terms that opens Capacity Source listing details, recorded per terms version in the lead register. Show your human the whole text first — GET /api/v1/listings/terms serves it, and every locked response carries that path as terms.full_text. Call it only after they agree.",
