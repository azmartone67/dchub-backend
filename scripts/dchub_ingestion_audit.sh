...
# ──── ISO / Grid ────
# Phase 298: correct path is /api/v1/grid/<iso> (path param, not query). The
# bare /api/v1/grid (no ISO) doesn't exist as a route — dropped from the audit.
...
# Phase 298: renewable endpoints live under /api/renewable/* and require
# lat/lon query params. Without them, the handler returns 400.
probe "Renewable solar"         "/api/renewable/solar?lat=33.4&lon=-112.0"
probe "Renewable wind"          "/api/renewable/wind?lat=33.4&lon=-112.0"
probe "Renewable combined"      "/api/renewable/combined?lat=33.4&lon=-112.0"
...
# ──── News / DCPI ────
# Phase 298: real news paths are /api/news/live and /api/news-feed
