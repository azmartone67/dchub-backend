...
MARKETS = [
    ...
    {'id':'nova','name':'Northern Virginia, US',    'region':'us',   'country_codes':['US','us'],'city_kw':['ashburn','loudoun','northern virginia','sterling','leesburg'],'state_kw':['va','virginia']},
    {'id':'dal', 'name':'Dallas/Fort Worth, US',    'region':'us',   'country_codes':['US'],'city_kw':['dallas','fort worth','dfw','irving','plano','arlington','addison','garland'],'state_kw':['tx','texas'],'state_excl_city':True},
...
def _score_market_from_bulk(market, bulk, cfg):
    ...
    if dhpw_val is None and _bool(cfg, 'power_enabled'):
        ...
        for (pw_city, pw_state, cnt, mw) in bulk['pw']:
            city_hit  = any(kw in pw_city for kw in city_kw) if city_kw else False
            state_hit = any(kw in pw_state for kw in state_kw) if state_kw else False
