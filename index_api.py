...
v5.0 — Load all facilities flat, match in Python, no state-filter issues
"""
...
# ─── 60 Global Markets ───────────────────────────────────────────────────────
# keywords: matched against LOWER(city) and LOWER(country) from facilities table
# country_codes: exact match on country column
MARKETS = [
    # US — unique states, so state_abbr is the primary filter; city_kw for disambiguation
    {'id':'nova','name':'Northern Virginia, US',    'region':'us',   'country_codes':['US','us'],'city_kw':['ashburn','loudoun','northern virginia','sterling','leesburg'],'state_kw':['va','virginia'],'power_kw':['northern_virginia','virginia','ashburn','va']},
    {'id':'dal', 'name':'Dallas/Fort Worth, US',    'region':'us',   'country_codes':['US'],'city_kw':['dallas','fort worth','dfw','irving','plano','arlington','addison','garland'],'state_kw':['tx','texas'],'state_excl_city':True,'power_kw':['dallas','fort_worth','dfw','tx','texas']},
    {'id':'phx', 'name':'Phoenix, US',              'region':'us',   'country_codes':['US'],'city_kw':['phoenix','chandler','mesa','tempe','scottsdale','gilbert'],'state_kw':['az','arizona'],'power_kw':['phoenix','chandler','az','arizona']},
    {'id':'chi', 'name':'Chicago, US',              'region':'us',   'country_codes':['US'],'city_kw':['chicago','elk grove','aurora','naperville','lisle'],'state_kw':['il','illinois'],'power_kw':['chicago','il','illinois']},
    {'id':'nyc', 'name':'New York/New Jersey, US',  'region':'us',   'country_codes':['US'],'city_kw':['new york','new jersey','secaucus','newark','jersey city','manhattan','brooklyn','bronx'],'state_kw':['nj','ny','new york','new jersey']},
    ...
    {'id':'atl', 'name':'Atlanta, US',              'region':'us',   'country_codes':['US'],'city_kw':['atlanta','norcross','lithia springs','douglasville','alpharetta'],'state_kw':['ga','georgia'],'power_kw':['atlanta','ga','georgia']},
    ...
    {'id':'iah', 'name':'Houston, US',              'region':'us',   'country_codes':['US'],'city_kw':['houston','katy','sugar land','stafford','webster'],'state_kw':[],'state_excl_city':True},
...
# ─── Bulk loader: minimal queries, flat row sets ─────────────────────────────
...
def _load_bulk(cfg, conn):
    ...
    # Flat rows: (country, state, city, mw)
    op_rows = _safe_q(cur,
        f"SELECT UPPER(COALESCE(country,'')), LOWER(COALESCE(state,'')), LOWER(COALESCE(city,'')), COALESCE(power_mw,0) FROM {fac} WHERE status IN ({op_ph})",
        op_st)
    pi_rows = _safe_q(cur,
        f"SELECT UPPER(COALESCE(country,'')), LOWER(COALESCE(state,'')), LOWER(COALESCE(city,'')), COALESCE(power_mw,0) FROM {fac} WHERE status IN ({pi_ph})",
        pi_st)

    # Deals: (market_lower, count, mw)
    deal_rows = _safe_q(cur,
        f"SELECT LOWER(COALESCE(market,'')), COUNT(*), COALESCE(SUM(mw),0) FROM {txn} WHERE date >= NOW() - INTERVAL '90 days' GROUP BY LOWER(market)")

    # Power plants: (city/market lower, state lower, count, mw)
    pw_rows = []
    if _bool(cfg,'power_enabled'):
        pw_rows = _safe_q(cur,
            f"SELECT LOWER(COALESCE({pcol},'')), LOWER(COALESCE({scol},'')), COUNT(*), COALESCE(SUM(capacity_mw),0) FROM {pw} GROUP BY LOWER({pcol}), LOWER({scol})")

    # Substations: (city lower, country lower, total_mva, avail_mva)
    sub_rows = []
    if _bool(cfg,'sub_enabled'):
        sub_rows = _safe_q(cur,
            f"SELECT LOWER(COALESCE(city,'')), LOWER(COALESCE(country,'')), COALESCE(SUM(capacity_mva),0), COALESCE(SUM(available_mva),0) FROM {sub} GROUP BY LOWER(city), LOWER(country)")

    # Market intelligence: (market lower, rate)
    mi_rows = []
    if _bool(cfg,'mi_enabled'):
        mi_rows = _safe_q(cur,
            f"SELECT LOWER(COALESCE(market,'')), avg_rate_per_kw FROM {mi} ORDER BY recorded_at DESC")
...
def _match_facilities(rows, market):
    """
    Match flat facility rows (country, state, city, mw) to a market.
    Strategy:
      - country must match market's country_codes
      - city OR state must match market's keywords
      - For intl markets with no state_kw, country match is sufficient
    """
    ...
    for (country, state, city, mw) in rows:
        ...
        # International markets (no state_kw, no specific city_kw) → country match is enough
        if not city_kw and not state_kw:
            total_cnt += 1
            total_mw  += float(mw)
        elif city_match:
            # City match always wins regardless of state
            total_cnt += 1
            total_mw  += float(mw)
        elif state_match and state:
            # State match only if state is populated (avoid matching empty-state rows to wrong market)
            total_cnt += 1
            total_mw  += float(mw)
...
def _score_market_from_bulk(market, bulk, cfg):
    ...
    if op_mw > 0:
        ...
        dhpi_d   = {
            'pipeline_mw':        round(pi_mw, 1),
            'operational_mw':     round(op_mw, 1),
            'pipeline_ratio_pct': round(pi_ratio * 100, 2),
        }
    ...
    for (mkt, cnt, mw) in bulk['deal']:
        hit = any(kw in mkt for kw in city_kw) if city_kw else any(c in mkt for c in codes)
        if hit:
            deal_cnt += int(cnt)
            deal_mw  += float(mw)
    ...
    if _bool(cfg, 'mi_enabled'):
        ...
        for (mkt, rate) in bulk['mi']:
            if mkt in seen or rate is None:
                continue
            hit = any(kw in mkt for kw in city_kw) if city_kw else False
            if hit:
                seen.add(mkt)
                baseline = 120.0
                ratio    = float(rate) / baseline
...
def init_config_table():
    try:
        ...
        for tbl, flag in [('substations','sub_enabled'),('discovered_power_plants','power_enabled'),
                          ('market_intelligence','mi_enabled')]:
            if tbl in existing:
...
@index_bp.route('/admin/diag')
def admin_diag():
    """Diagnose what state/city values exist for US facilities"""
    cfg = _get_config()
    err = _require_admin(cfg)
    if err: return err
    try:
        ...
        # Top states for US
        cur.execute(f"""
            SELECT COALESCE(state,'NULL'), COUNT(*), COALESCE(SUM(power_mw),0)
            FROM {fac} WHERE country IN ('US','us') AND status IN ({op_ph})
            GROUP BY state ORDER BY COUNT(*) DESC LIMIT 20
        """, op_st)
        us_states = [{'state': r[0], 'count': r[1], 'mw': round(float(r[2]),1)} for r in cur.fetchall()]

        # Top cities in Virginia
        cur.execute(f"""
            SELECT COALESCE(city,'NULL'), COALESCE(state,'NULL'), COUNT(*), COALESCE(SUM(power_mw),0)
            FROM {fac} WHERE country IN ('US','us') AND status IN ({op_ph})
            AND (LOWER(state) IN ('va','virginia') OR LOWER(city) LIKE '%ashburn%' OR LOWER(city) LIKE '%loudoun%')
            GROUP BY city, state ORDER BY COUNT(*) DESC LIMIT 30
        """, op_st)
        va_cities = [{'city': r[0], 'state': r[1], 'count': r[2], 'mw': round(float(r[3]),1)} for r in cur.fetchall()]

        # Total US operational
        cur.execute(f"SELECT COUNT(*), COALESCE(SUM(power_mw),0) FROM {fac} WHERE country IN ('US','us') AND status IN ({op_ph})", op_st)
        row = cur.fetchone()
        us_total = {'count': row[0], 'mw': round(float(row[1]),1)}

        # Power plants sample for VA
        pcol = cfg.get('power_city_col', 'market')
        scol = cfg.get('power_state_col', 'state')
        pw   = cfg.get('power_table', 'discovered_power_plants')
        cur.execute(f"""
            ...
            FROM {pw}
            WHERE LOWER({scol}) IN ('va','virginia') OR LOWER({pcol}) LIKE '%virginia%' OR LOWER({pcol}) LIKE '%ashburn%'
            GROUP BY LOWER({pcol}), LOWER({scol}) LIMIT 20
        """)
        pw_va = [{'market': r[0], 'state': r[1], 'count': r[2], 'mw': round(float(r[3]),1)} for r in cur.fetchall()]
