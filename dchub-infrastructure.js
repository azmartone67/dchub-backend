...
(function() {
    ...
    const CONFIG = {
        ...
        HIFLD: {
            ...
            gasPipelines: 'https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/Natural_Gas_Pipelines/FeatureServer/0',
    ...
    const DataLoader = {
        ...
        async queryArcGIS(baseUrl, options = {}) {
            // Build geometry with required spatialReference for HIFLD API
            let geometryParams = {};
            if (options.geometry) {
                // Use bbox format instead of JSON for ArcGIS compatibility
                geometryParams = {
                    geometry: `${options.geometry.xmin},${options.geometry.ymin},${options.geometry.xmax},${options.geometry.ymax}`,
            ...
            const params = new URLSearchParams({
                ...
                f: 'geojson',
                resultRecordCount: options.limit || CONFIG.MAX_RECORDS_PER_REQUEST,
                ...geometryParams
            });
            ...
            try {
                ...
                if (!response.ok) {
                    // Silently fail for 400 errors - use fallback data
                    if (response.status === 400) {
                        console.warn('📡 HIFLD API returned 400, using fallback data');
                        return null;
                    }
                ...
                // Suppress noisy errors - just log warning
                console.warn('📡 HIFLD API unavailable, using fallback:', error.message);
                return null;
            }
        },
        
        // Load substations for visible map area
        async loadSubstations(bounds) {
            ...
            return this.queryArcGIS(CONFIG.HIFLD.substations, {
                geometry,
        ...
        // Load transmission lines for visible area
        async loadTransmissionLines(bounds) {
            ...
            return this.queryArcGIS(CONFIG.HIFLD.transmissionLines, {
                geometry,
        ...
        async loadGasPipelines(bounds) {
            ...
            return this.queryArcGIS(CONFIG.HIFLD.gasPipelines, {
                geometry,
        ...
        async loadPowerPlants(bounds) {
            ...
            return this.queryArcGIS(CONFIG.HIFLD.powerPlants, {
                geometry,
    ...
    const LayerRenderer = {
        ...
        // Render substations from GeoJSON with enhanced capacity data
        renderSubstations(geojson, layerGroup) {
        ...
        // Render transmission lines from GeoJSON
        renderTransmissionLines(geojson, layerGroup) {
            if (!geojson || !geojson.features) return 0;
        ...
        // Render gas pipelines from GeoJSON with capacity
        renderGasPipelines(geojson, layerGroup) {
            if (!geojson || !geojson.features) return 0;
            ...
            geojson.features.forEach(feature => {
                ...
                const capacityInfo = pipelineInfo ? 
                    `<div style="margin-top:6px;padding:6px 8px;background:rgba(249,115,22,0.1);border-radius:6px;font-size:11px">
    ...
    const DynamicLayerManager = {
        ...
        init(map, existingLayers) {
            ...
            // Add map move listener for dynamic loading
        ...
        async loadHIFLDSubstations(bounds) {
            this.showLoading('Loading 70,000+ substations...');
        ...
        async loadHIFLDGas(bounds) {
            this.showLoading('Loading 300,000+ miles gas pipelines...');
