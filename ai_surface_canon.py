...
PINNED = {
    ...
    "tools_advertised": 85,                   # canonical advertised count == live tools/list (85 as of 2026-09-07: mcp-server #368 SPLIT the single standing_intent into register_standing_intent / list_standing_intents / delete_standing_intent, +2 net -> 85; verified live via a tools/list handshake against _MCP_BASE. canon_text() renders {canon_tools} from THIS pin with NO live fallback, so /llms.txt + /AGENTS.md served "83 tools" until this bump even though resolve_canon()/api/v1/canon/phrases already returned 85. 83 as of 2026-08-30: +summarize_for_citation, found by the drift guard — the tool went live on the MCP server and the pin did not follow, so every discovery surface under-reported by one; 82 as of 2026-07-31: +get_power_availability_timeline, gateway v2.10.0; 81 as of 2026-07-29: +get_hosting_capacity; 80 as of 2026-07-26: +execute_plan; 79 as of 2026-07-20: +get_global_power/get_permitting_intel/plan_query/research_task/simulate_scenario/standing_intent). PINNED fallback; resolve_canon() overrides it with the live count probed from _MCP_BASE (the public gate) so consumers never go stale. /AGENTS.md reads PINNED directly, so keep current. ★MUST equal len(tool_manifest) — tests/test_fix_closure_shell.py asserts it, so a count edit that skips the manifest cannot land.
    ...
    "tool_manifest": [
        ...
        "register_standing_intent", "research_task",
        ...
        "simulate_scenario", "site_selection_canvas",
