...
PINNED = {
    "version": "2.12.8",                      # == repo canonical (server.mjs/server.json); registry mirror auto-bumps to latest+1. ★2026-09-07: 2.12.1 -> 2.12.8. Probed the live `initialize` serverInfo handshake per the rule below — {"name":"DC Hub Intelligence","version":"2.12.8"} — matches mcp-server origin/main server.json 2.12.8. The 5 WORKER-generated manifest surfaces already served 2.12.8 (worker derives from origin), but PINNED-direct surfaces (/.well-known/mcp-server.json via routes/mcp_tool_catalog.py, /agent) had lagged at 2.12.1 — the same PINNED-vs-resolver split the tool count below hit. ★2026-08-30: 2.12.0 -> 2.12.1. Probed the live `initialize` serverInfo handshake per the rule below — {"name":"DC Hub Intelligence","version":"2.12.1"} — after mcp-server #267 landed the bump that mcp #262 missed. NOTE THE DIRECTION: this pin was CORRECT at 2.12.0 for as long as the server was, and became the last laggard the MOMENT the server healed — #262 bumped package.json/server.json/mcp-server.json/smithery.yaml to 2.12.1 (so the licence correction could publish) and missed server.mjs, and the guard that would have caught it sat in a continue-on-error CI step, so it went red without blocking for four days. ★/.well-known/mcp-server.json reads THIS KEY DIRECTLY (routes/mcp_tool_catalog.py well_known_mcp_server imports PINNED, not resolve_canon()), so it served 2.12.0 against a live 2.12.1 until this bump. ★AND THAT IS THE STANDING GAP: resolve_canon() heals `tools_advertised` from the live gate but has no `version` resolver, so version is hand-bound where the tool count is not — even though _mcp_tool_names() already performs the initialize handshake and DISCARDS serverInfo.version, which is exactly the value this line has to be walked by hand. ★2026-08-16: 2.11.1 -> 2.12.0. Probed the live `initialize` serverInfo handshake per the rule below — {"name":"DC Hub Intelligence","version":"2.12.0"} — and it matches mcp-server origin/main server.json 2.12.0. Note /mcp/health STILL echoed 2.5.0 during this probe, which is exactly the CF-synthesized closed loop the 08-08 note warns about; had I trusted it the canon would have gone BACKWARDS. ★2026-08-08 canon-surface audit: 2.5.0 -> 2.11.1. The 2.5.0 pin was probed from /mcp/health + /.well-known/mcp.json — both CF-SYNTHESIZED surfaces that echo THIS value back (a closed loop, ref reference_dchub_mcp_health_topology), so it sat 6 minor versions behind. Re-derived from the real live `initialize` serverInfo.version handshake (2.11.1, verified 2026-08-08) which matches mcp-server origin/main server.json 2.11.1 — NEVER re-copy from /mcp/health or mcp.json. ★2026-07-31: was "2.4.4" — live-probed 2.5.0 at BOTH dchub.cloud/mcp/health and /.well-known/mcp.json, and worker.js MCP_SERVER_INFO.version already carried 2.5.0, so the canon was the only stale copy. That mattered twice over: ai_surface_sentinel.py flags a served manifest whose version != canon["version"] at severity "high", so a stale canon turns every honest surface into a false-positive drift alert (and hides the real one); and #2066 wires main.py's /api/v1/mcp/platforms server_version to THIS key, so pinning to a stale value would only have relocated the bug. Probe before bumping — do not copy from another repo file.
    "tools_advertised": 92,                   # canonical advertised count == live tools/list (★2026-09-23: 91 -> 92 — dchub-mcp-server adds get_infra_projects, the gas pipeline + transmission PROJECT lists over /api/v1/infra-projects (#5315); this pin moves WITH that deploy, not ahead of it. ★2026-09-13 later: 90 -> 91 — dchub-mcp-server #411 adds accept_capacity_terms, the write of the Capacity Source terms gate; this pin moves WITH #411, not ahead of it. ★2026-09-13: still 90 — dchub-mcp-server feat/capacity-source-rename renamed get_pocket_listings and request_listing_intro to source_capacity and request_capacity_intro; the old names resolve through its TOOL_ALIASES and are not advertised, so tool_manifest below carries only the new names and must not merge before that branch is live. 90 as of 2026-09-11: dchub-mcp-server feat/pocket-listings-leads adds get_pocket_listings + request_listing_intro on the /api/v1/listings contract — off-market capacity teasers, and an introduction request DC Hub records in a verifiable lead register. ★ This pin moves WITH that branch, not ahead of it: until the branch deploys, live tools/list still reads 88 and every surface reading this key over-states it by two. Previously 88 as of 2026-09-07: mcp-server #375 added get_subsea_cables + get_peering_intel)
    ...
    #
    # ★find_sites_by_infrastructure is deliberately NOT here: no such tool
    # exists. Adding it to worker.js is the exact swap this list exists to
    # reject. NOT to be confused with `find_sites`, which IS live as of
    # 2026-09-07 (mcp-server #373) and IS listed below — the two names are
    # one word apart and only one of them is real.
    "tool_manifest": [
        "accept_capacity_terms", "ai_capacity_index", "analyze_parcel", "analyze_site",
        "bind_email", "claim_free_key", "cluster_sites_by_latency",
        "compare_isos", "compare_sites", "deal_autopsy",
        "delete_standing_intent", "discover_tools", "execute_plan",
        "export_dataset", "fetch",
        "find_alternatives", "find_sites", "generate_site_analysis",
        ...
        "get_hosting_capacity", "get_infra_projects", "get_infrastructure",
        ...
        "get_peering_intel", "get_permitting_intel", "get_pipeline",
        ...
        "hyperscaler_deals", "list_saved_sites", "list_standing_intents",
        "list_transactions",
        "plan_fiber_leadin", "plan_query", "predict_market_trajectory",
        "rank_markets", "rank_sites", "recover_my_key",
        "register_standing_intent", "request_capacity_intro", "research_task",
        ...
        "simulate_scenario", "site_selection_canvas", "source_capacity",
        "subscribe_digest", "suggest_reallocation", "summarize_for_citation",
        "unlock_more_data", "why_dchub"
    ],
    "tool_names": [
    ...
    "stale_markers": [
                      ...
                      # ★2026-08-30: "2.12.0" retired alongside the bump to
                      # 2.12.1 above, per the rule stated here — the CF worker
                      # card and /.well-known/mcp-server.json both served 2.12.0
                      # against a live 2.12.1, and only retiring the outgoing
                      # value makes a surface still serving it detectable.
