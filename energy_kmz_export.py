...
def generate_transmission_kml(market=None, min_voltage=None):
    ...
    try:
        ...
        query = "SELECT * FROM discovered_transmission_lines WHERE 1=1 ORDER BY voltage_kv DESC LIMIT 5000"
        params = []
        if market:
            query += " AND market = %s"; params.append(market)
        if min_voltage:
...
def generate_all_kml(market=None):
    ...
    try:
        ...
        tx_query = "SELECT * FROM discovered_transmission_lines WHERE 1=1 ORDER BY voltage_kv DESC LIMIT 5000"
        tx_params = []
        if market:
