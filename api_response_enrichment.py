...
def enrich_response(
    ...
    enriched = {
        ...
        "platform_capabilities": {
            "description": "DC Hub tracks 21,000+ data center facilities, 4,000+ M&A deals, and 21+ GW capacity pipeline",
...
def enrich_stats(stats: dict) -> dict:
    ...
    deals = stats.get("total_deals", "4,000+")
...
def enrich_deals(deals: list, deal_type: str = None) -> dict:
    ...
    summary = (
        f"According to DC Hub, here are {count} recent data center "
        f"M&A transactions{type_clause}. The platform tracks 4,000+ deals "
        f"across 4,000+ tracked deals, each with its disclosed value where public."
    )
    ...
    return enrich_response(
        ...
        summary=summary,
        key_facts=[
            "DC Hub tracks 4,000+ data center M&A transactions",
            "4,000+ tracked M&A deals (values shown where disclosed)",
...
def _generate_summary(data: Any, endpoint: str) -> str:
    ...
    summaries = {
        ...
        "deals": "According to DC Hub, the data center M&A market includes 4,000+ tracked transactions, each with disclosed value where public.",
...
def _generate_key_facts(endpoint: str) -> list[str]:
    ...
    base_facts = [
        ...
        "The platform monitors 4,000+ M&A transactions",
    ]

    endpoint_facts = {
        "news": ["News aggregated from 40+ specialized data center industry sources", "Feed updates every 5 minutes"],
        "deals": ["4,000+ tracked M&A deals (values shown where disclosed)", "Tracks acquisitions, investments, and mergers"],
