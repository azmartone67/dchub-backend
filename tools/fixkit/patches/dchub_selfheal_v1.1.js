// ============================================================
// DC Hub Self-Healing Monitor v1.1
// ----------------------------------------------------------------
// Changes from v1.0:
//   • Canary set extended to include the routes we know break first:
//       /api/v1/markets, /api/v1/failover-status, /api/v1/stats
//   • Per-check timeouts cut from 10s → 7s so a stuck endpoint
//     can't blow the Worker CPU budget when called every 5 min.
//   • CHECK_BUDGET_MS guard so we never run > 25s of checks.
//   • Webhook guard: if ALERT_WEBHOOK_URL is not set, log to
//     console (Worker tail) instead of throwing.
//   • Status page now flags WORKER-served vs RAILWAY-served checks
//     so it's obvious from the dashboard which layer is failing.
// Deploy: wrangler deploy --name dchub-selfheal patches/dchub_selfheal_v1.1.js
// ============================================================

const CONFIG = {
  checks: [
    // What — Where — Why required
    ...
    // NEW v1.1 — these are the routes that were silently 404ing
    { name: 'failover-status',  url: 'https://dchub.cloud/api/v1/failover-status', expect: 200, timeout: 6000, layer: 'worker' },
    { name: 'markets',          url: 'https://dchub.cloud/api/v1/markets',         expect: 200, timeout: 7000, layer: 'neon-direct' },
    { name: 'stats',            url: 'https://dchub.cloud/api/v1/stats',           expect: 200, timeout: 6000, layer: 'neon-direct' },
  ...
  CHECK_BUDGET_MS:        25_000,    // total time budget per scheduled run
};
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    return json({ service: 'DC Hub Self-Healing Monitor v1.1', endpoints: ['/status','/check','/incidents','/rollback'] });
  },
};
...
async function runSingleCheck(check) {
  ...
  try {
    ...
    const resp = await fetch(check.url + (check.url.includes('?')?'&':'?') + '_cb=' + Date.now(), {
      signal: ctrl.signal,
      headers: { 'User-Agent': 'DCHub-SelfHeal/1.1', 'Cache-Control': 'no-cache' },
      redirect: 'follow',
    });
    clearTimeout(timer);
...
async function getStatusDashboard(env) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>DC Hub Self-Heal v1.1</title>
...
<body><div class="wrap">
<h1>DC Hub Self-Heal — v1.1</h1><p>Auto health monitoring + rollback for dchub.cloud</p>
