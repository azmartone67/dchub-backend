<!-- fingerprint:3d96f7fcc3effd31664481f75b82e936 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=TVA). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100541). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-05T17:08:10.814584Z · inv #100541_

## The approved recommendation

Open routes/iso_orchestrator.py and trace the scheduled write loop feeding grid_data for iso=TVA, then check the orchestrator's last-run logs and the upstream feed status for the past 24h to determine whether the job stopped firing or the upstream returned no rows.
...
`grid_data: iso=TVA` — is enumerated in that doc's rolled-up roster, so closing
