<!-- fingerprint:551d25518d6d304577ce73c5e6a295b6 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=FPC). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100545). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-05T17:08:02.591057Z · inv #100545_

## The approved recommendation

Open routes/iso_orchestrator.py, trace the FPC feed's aggregate fan-out registration and its last-run/scheduler state, and manually re-trigger the ingest loop for FPC plus the 8 ISOs currently flagged in brain_findings (APS, PSCO, CHPD, GCPD, BPA, TVA, AVA, SC) to backfill the missing 24h window, then confirm fresh grid_data writes appear.
...
`grid_data: iso=FPC` — is enumerated in that doc's rolled-up roster, so closing
