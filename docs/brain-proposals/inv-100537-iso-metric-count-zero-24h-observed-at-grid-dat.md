<!-- fingerprint:e672c1f97548d6e3bc3050e8d773821a -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=PJM). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100537). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-05T17:08:23.032514Z · inv #100537_

## The approved recommendation

Open routes/iso_orchestrator.py, locate the PJM (and fan-out aggregate) feed registration and its scheduling/cron entry, and check the last successful run timestamp and any auth/upstream-API error for the PJM ingest job in the past 24h to confirm whether the loop is dead vs. the PJM upstream API is failing.
...
`grid_data: iso=PJM` — is enumerated in that doc's rolled-up roster, so closing
