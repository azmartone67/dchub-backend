<!-- fingerprint:f4a8972a6ebc11bf11734410364f8553 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=AECI). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100486). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:33.968736Z · inv #100486_

## The approved recommendation

Open routes/iso_orchestrator.py and trace the ISO code 'AECI' to its actual ingest source (standalone module vs aggregate fan-out feed), then check that source's most recent grid_data write timestamp and its scheduling workflow to determine whether the loop stopped (code/schedule) or AECI is legitimately sparse.
...
`grid_data: iso=AECI` — is enumerated in that doc's rolled-up roster, so closing
