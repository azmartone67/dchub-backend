<!-- fingerprint:baa91f4428357f72fa69b448606ac45c -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=SCEG). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100544). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-05T17:08:07.565310Z · inv #100544_

## The approved recommendation

Open routes/iso_orchestrator.py, locate the fan-out feed that registers SCEG's grid_data writes, and restart/re-trigger that ingest loop, then confirm a fresh grid_data write timestamp for iso=SCEG within the last hour before closing the finding.
...
`grid_data: iso=SCEG` — is enumerated in that doc's rolled-up roster, so closing
