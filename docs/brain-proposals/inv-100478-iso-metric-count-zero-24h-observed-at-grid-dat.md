<!-- fingerprint:b603625c2e6439df9ce8ba5d15a976ea -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=PACE). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100478). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:59.933776Z · inv #100478_

## The approved recommendation

Open routes/iso_orchestrator.py and trace the PACE code to its owning fan-out feed/workflow, then check that workflow's last successful grid_data write timestamp to confirm whether the ingest loop stalled (restart it) or PACE was decommissioned (suppress the detector).
...
`grid_data: iso=PACE` — is enumerated in that doc's rolled-up roster, so closing
