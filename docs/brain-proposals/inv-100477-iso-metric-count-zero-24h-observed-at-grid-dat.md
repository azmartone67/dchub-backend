<!-- fingerprint:f768c2981e80ae426e23807ab57f3b9a -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=GVL). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100477). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:07:00.932237Z · inv #100477_

## The approved recommendation

Open routes/iso_orchestrator.py and resolve the ingest module/workflow that GVL's fan-out feed registers under, then inspect that collector's last-run logs to confirm whether the scheduled job stalled or errored, and manually re-trigger it to restore grid_data writes for GVL.
...
`grid_data: iso=GVL` — is enumerated in that doc's rolled-up roster, so closing
