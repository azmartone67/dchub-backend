<!-- fingerprint:d9ec03b23d245b1b223a518143b65cff -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=PNM). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100481). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:49.779614Z · inv #100481_

## The approved recommendation

Open routes/iso_orchestrator.py, look up the ingest module/workflow that registers the PNM feed (accounting for aggregate fan-out codes), confirm PNM belongs to the tracked live-ISO set, and re-run that ingest loop to restore grid_data writes for iso=PNM.
...
`grid_data: iso=PNM` — is enumerated in that doc's rolled-up roster, so closing
