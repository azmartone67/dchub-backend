<!-- fingerprint:01d81eeaa44991cd45f2abc4cc90cecf -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=GCPD). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100483). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:45.183457Z · inv #100483_

## The approved recommendation

Open routes/iso_orchestrator.py and resolve which ingest module/workflow registers the GCPD feed (it is likely a fan-out under an aggregate code), then check that workflow's scheduler/last-run logs to confirm whether the loop stalled or the upstream feed went silent.
...
`grid_data: iso=GCPD` — is enumerated in that doc's rolled-up roster, so closing
