<!-- fingerprint:58a9ad5b403b21e0a04aed74145f9b3e -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=EU_BE). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100603). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-11T04:11:01.024382Z · inv #100603_

## The approved recommendation

Re-run/verify the EU ENTSOE ingestion workflow (the scheduled job feeding routes/iso_eu_entsoe.py / iso_eu_be) and confirm its ENTSOE API credentials and last-success timestamp, then check whether a fresh row appears in grid_data for iso=EU_BE within one cycle.
...
`grid_data: iso=EU_BE` — is enumerated in that doc's rolled-up roster, so closing
