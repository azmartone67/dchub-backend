<!-- fingerprint:2c3ca494f240fba2d809ee2546997a2f -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_dropped (observed at: grid_data: iso=TPWR). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100518). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-04T21:39:38.517296Z · inv #100518_

## The approved recommendation

Open routes/iso_orchestrator.py and trace which aggregate fan-out feed TPWR (and the co-affected TIDC/SPA/TAL) register under, then inspect that feed's upstream API call for per-metric errors causing the partial 1-of-5-15 write.
...
`grid_data: iso=TPWR` — is enumerated in that doc's rolled-up roster, so closing
