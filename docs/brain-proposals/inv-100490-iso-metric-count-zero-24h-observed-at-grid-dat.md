<!-- fingerprint:11e9d5f064e83faf9cbb0f583747a999 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=SEC). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100490). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:21.724462Z · inv #100490_

## The approved recommendation

Open routes/iso_orchestrator.py and grep for the SEC registration to resolve the concrete ingest module and its scheduled workflow, then check that module's last successful grid_data INSERT timestamp and re-trigger the SEC ingestion run.
