<!-- fingerprint:954dcb59c859b096fa977444d22bffaf -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — mcp_tool_sunset_candidate (observed at: tool:find_alternatives). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100654). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-15T21:29:03.685417Z · inv #100654_

## The approved recommendation

Query the mcp_tool_calls error/log table for tool='find_alternatives' over the last 7 days to confirm whether the zero run-rate is silent errors or organic traffic decline before any sunset decision; no code change to any routes/*.py file is warranted.
...
`tool:find_alternatives` — is enumerated in that doc's rolled-up roster, so closing
