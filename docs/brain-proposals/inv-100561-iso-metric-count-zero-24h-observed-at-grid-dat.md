<!-- fingerprint:f0cdd25f15fb750be7aed61ba63e8ed3 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=CPLE). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100561). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-07T20:34:05.868385Z · inv #100561_

## The approved recommendation

Open routes/iso_orchestrator.py and trace the aggregate fan-out feed that registers CPLE, then check its scheduler/last-run timestamp and manually re-trigger that feed's ingest loop; confirm a fresh grid_data write appears for iso=CPLE before closing the finding.
...
`grid_data: iso=CPLE` — is enumerated in that doc's rolled-up roster, so closing
