<!-- fingerprint:41cb88f14db771c9c47ab60bd0b683cd -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=EU_IT_NORD). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100441). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-02T03:06:00.665569Z · inv #100441_

## The approved recommendation

Inspect the ENTSOE ingestion workflow and its module (iso_eu_*.py family / shared entsoe fetcher) plus its API-credential env var, checking git log around PR #3538's env-only secret change and the last successful grid_data write timestamp for iso=ENTSOE, to confirm whether a missing/expired ENTSOE key silenced the shared EU loop.
...
`grid_data: iso=EU_IT_NORD` — is enumerated in that doc's rolled-up roster, so closing
