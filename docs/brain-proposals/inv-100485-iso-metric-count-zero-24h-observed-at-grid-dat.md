<!-- fingerprint:cdef563028eae7fd3a14a790eb65e320 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=PSCO). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100485). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:35.509455Z · inv #100485_

## The approved recommendation

Open routes/iso_orchestrator.py and resolve which ingest module/workflow feeds ISO PSCO, then query grid_data for PSCO's most recent write timestamp to confirm the loop stalled (last write >24h ago) versus PSCO never having history.
...
`grid_data: iso=PSCO` — is enumerated in that doc's rolled-up roster, so closing
