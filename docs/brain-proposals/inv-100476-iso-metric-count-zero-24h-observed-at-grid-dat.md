<!-- fingerprint:dc88daf33f9596d7f65d6299a54fe770 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=SOCO). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100476). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:07:05.034139Z · inv #100476_

## The approved recommendation

Open routes/iso_orchestrator.py and resolve which workflow/module registers the SOCO feed (checking whether it fans out under an aggregate code), then inspect that module's last successful write timestamp and restart the stalled SOCO ingestion loop.
...
`grid_data: iso=SOCO` — is enumerated in that doc's rolled-up roster, so closing
