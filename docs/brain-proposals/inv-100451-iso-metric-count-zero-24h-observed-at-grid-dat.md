<!-- fingerprint:88d4b84be73539cc295a40d769538345 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=EU_HU). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100451). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-02T20:40:09.530503Z · inv #100451_

## The approved recommendation

Inspect the scheduled European ISO ingest worker/workflow logs for the last 24h (the job that writes grid_data for the EU_* ISOs including EU_HU) to confirm whether the loop is erroring or simply not scheduled, since EU_HU, EU_IT_CNOR, EU_DE_LU, EU_DK_1 and EU_DK_2 all went zero together.
...
`grid_data: iso=EU_HU` — is enumerated in that doc's rolled-up roster, so closing
