<!-- fingerprint:2dc32ef8d78cbf7f641b254e52b0b483 -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — facility_duplicates_unmarked (observed at: /api/v1/admin/facility-dedup/analyze?country=ID). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100626). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-12T10:56:32.445503Z · inv #100626_

## The approved recommendation

Call POST /api/v1/admin/facility-dedup/apply?country=ID&confirm=1 to collapse the currently-unmarked ID cross-source duplicate rows, after first GET-ing /api/v1/admin/facility-dedup/analyze?country=ID to confirm the candidate pairs are true duplicates.
...
`/api/v1/admin/facility-dedup/analyze?country=ID` — is enumerated in that doc's rolled-up roster, so closing
