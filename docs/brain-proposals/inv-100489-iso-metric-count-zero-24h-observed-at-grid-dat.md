<!-- fingerprint:7749ea32b12bb07dde1138d88baf443d -->
**SPEC-ONLY** — this PR changes no running code and is not a fix; it captures an approved recommendation as an implementable spec.

# Brain proposal — iso_metric_count_zero_24h (observed at: grid_data: iso=TIDC). What is the root cause, and is there a single unambiguous find-and-replace fix in one file that resolves it?
...
> Auto-captured from an **approved** brain inv item (#100489). The brain's
> recommendation couldn't be expressed as a single-file edit, so it's filed here
> as a spec for a human to implement (or discard). **Draft PR — a human merges.**

_Filed 2026-09-03T19:06:24.759924Z · inv #100489_

## The approved recommendation

Open routes/iso_orchestrator.py, find the aggregate feed registration that maps to iso=TIDC, identify its scheduled workflow, and manually re-trigger that ingestion workflow while capturing its run logs to confirm whether the failure is internal (loop halted) or upstream (source API empty).
...
`grid_data: iso=TIDC` — is enumerated in that doc's rolled-up roster, so closing
