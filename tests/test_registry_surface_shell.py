...
def test_rank_claim_reports_unverified_rather_than_inventing_a_number():
    """The lane must not manufacture a rank. Scoring position from an unverified
    source is the exact class of confident, unfounded metric the preceding shells
    were spent removing."""
    from routes.registry_surface_shell import _lane_rank_claim
    checks = _lane_rank_claim()
    assert checks and all(c["status"] == "INDETERMINATE" for c in checks), (
        "the rank lane reports a verdict it cannot support — it must stay "
        "INDETERMINATE until a real position probe exists")
