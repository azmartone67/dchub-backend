    ...
    for fn in tree.body:
        ...
        for n in ast.walk(fn):
            if (isinstance(n, ast.Constant) and isinstance(n.value, str)
                    and id(n) not in prose and _GRANT_SQL_RE.search(n.value)):
                hits.add(fn.name)
    return hits
...
def test_nothing_but_grant_post_can_write_a_grant():
    ...
    hits = _funcs_with_grant_sql(ROOT / "routes" / "squasher_action_classes.py")
    assert hits == {"grant_post"}, hits


def test_CONTROL_the_grant_scanner_still_sees_a_grant_hidden_in_any_function(tmp_path):
    """The control for the exclusion above: prose is skipped, an execute() is
    not. A grant planted in graduation_report's BODY is caught."""
    src = (ROOT / "routes" / "squasher_action_classes.py").read_text(encoding="utf-8")
    planted = src.replace(
        "def file_graduation_proposals(",
        "def _planted_grant(cur):\n"
        "    cur.execute(\"UPDATE brain_action_classes SET granted = TRUE\")\n\n\n"
...
def test_actuate_with_confirm_fires_under_the_shells_budget_and_ledgers_the_rollback(monkeypatch):
    ...
    ins = _sqls(cur, "INSERT INTO brain_actuator_runs")
...
def test_actuate_captures_the_news_pre_image_as_the_rollback(monkeypatch):
    ...
    cur = _Cur({K_PRE_IMAGE: [(1, "unknown"), (2, "rejected"), (3, None)],
                K_POST_IMAGE: [(1,), (3,)],
                "INSERT INTO brain_actuator_runs": [(78,)]})
    monkeypatch.setattr(sac, "_conn", lambda: _Conn(cur))
    monkeypatch.setenv("ACTION_CLASSES_ENABLED", "1")
    body, code = sac.actuate(NEWS, confirm=True)
    ...
    p = _sqls(cur, "INSERT INTO brain_actuator_runs")[0][1]
    assert json.loads(p[5]) == [{"id": 1, "prior": "unknown"}, {"id": 3, "prior": None}]
...
@pytest.mark.parametrize("name", ["squasher_action_classes.py", "squasher_queue.py"])
def test_no_kill_switch_block_returns_5xx(name):
    assert _kill_5xx(ROOT / "routes" / name) == []
