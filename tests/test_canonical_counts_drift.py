...
def test_capabilities_quotable_never_contradicts_its_own_counts():
    ...
    # ★2026-09-20: facilities moved with the ninth walk (22,031 -> 24,449, the
    # live /api/v1/stats reading that day). FOURTH time, same cause, and the
    # 09-11 note below already states it: with the pin walked to 24,400+ the
    # old fixture sat UNDER _canon_facilities_floor(), the feed correctly
    # popped the field, and this failed `counts.facilities is None` while
    # passing on origin/main. The fixture tracks the floor by design.
...
KNOWN_STALE_COUNT_DEBT = {
    ...
    # painters each hard-coded a stale facilities floor; they now emit
    # through _market_offer_html, which quotes no facility total at all —
    # the per-market copy uses that page's own measured count. Debt paid,
    # so the entry goes, per this file's own reconciliation rule.
...
def test_inverted_fence_covers_more_than_the_allow_list():
    ...
    # facilities_stale_floor. Its THREE market-page CTA painters each
    # hard-coded the same stale facility total; they now emit through one
    # builder whose copy quotes no repo-wide total at all — each page uses
