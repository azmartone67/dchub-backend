...
mocked routes.brain_rag (no Flask, no DB, no Cohere):
  ...
  thresholded, undisclosed values count as compatible, kill switch.
...
def test_undisclosed_value_still_dup(monkeypatch):
    """One/both values undisclosed → compatible → dup on high cosine."""
    _install_fake_brain_rag(monkeypatch, lambda q, k=8, corpus=None: [CORWEAVE_HIT],
                            fake_embed_ok)
...
def test_values_compatible():
    assert dis._values_compatible(None, 9e9, 0.15)          # undisclosed new
    assert dis._values_compatible(9e9, None, 0.15)          # undisclosed cand
    assert dis._values_compatible(9e9, 9.5e9, 0.15)         # within 15%
    assert not dis._values_compatible(2e9, 9e9, 0.15)       # clearly different
