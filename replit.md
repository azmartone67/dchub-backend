...
-   **Jobs API (Feb 2026):** All background tasks converted to one-shot POST `/api/jobs/*` endpoints with `DCHUB_ADMIN_KEY` auth. 18 job endpoints: news-refresh, discovery, global-intel (alias: global-intelligence), ecosystem (alias: ai-ecosystem), evolution, outreach (alias: ai-outreach), promotion, content-publish, ai-wars, auto-approve, keep-alive, autopilot, autonomous-brain, alert-emails, simple-alerts, market-report, infrastructure-sync, energy-discovery, capacity-headroom, ambassador. Only health watchdog, Neon keepalive, and periodic GC remain as in-process daemon threads.
