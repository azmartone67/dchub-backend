...
-   **Jobs API (Feb 2026):** All background tasks converted to one-shot POST `/api/jobs/*` endpoints with `DCHUB_ADMIN_KEY` auth. 19 job endpoints: news-refresh, discovery, global-intel (alias: global-intelligence), ecosystem (alias: ai-ecosystem), evolution, outreach (alias: ai-outreach), promotion, content-publish, ai-wars, auto-approve, keep-alive, autopilot, autonomous-brain, alert-emails, simple-alerts, market-report, infrastructure-sync, energy-discovery, capacity-headroom, ambassador, db-backup. Only health watchdog, Neon keepalive, and periodic GC remain as in-process daemon threads.
-   **Database Backup System (Mar 2026):** `db_backup.py` exports 18 critical+secondary tables from Neon to compressed JSON files (`backups/` dir). Keeps 7 daily + 4 weekly backups locally. Optional R2 cloud upload if credentials configured. Triggered via `POST /api/jobs/db-backup` (admin key), `GET /api/jobs/db-backup/list`, `GET /api/jobs/db-backup/verify`.
-   **Self-Healing Server (Mar 2026):** `start_server.sh` wraps gunicorn with syntax pre-check, auto-restart on crash (5s delay, max 10/hour), and MCP server lifecycle management. Health watchdog (`health_watchdog.py`) checks API response, DB connectivity, memory, disk, threads every 60s — force-restarts after 3 consecutive failures.
-   **Discovery Monitoring API (Feb 2026):** 6 admin endpoints at `/api/admin/discovery/*` for monitoring autodiscovery pipeline, scheduler health, discovery queue, metrics, AI platform tracking, and manual discovery triggers. All require `DCHUB_ADMIN_KEY` auth. New table: `facility_count_history` for growth tracking.
...
-   **Health endpoint:** `/api/health/db` returns real-time pool stats, circuit breaker state, memory usage, leaked connections detail, active checkout count, and Neon connection limits. Memory `warning` flag fires above `_HEALTH_MEMORY_THRESHOLD_MB = 2048` (Apr 2026, raised from 512). Periodic GC cache-clear trigger `_MEMORY_LIMIT_MB = 1024` (raised from 256).
-   **Memory diagnostics endpoint (Apr 2026):** `GET /api/admin/memory-top` (header `X-Admin-Key: $DCHUB_ADMIN_KEY`). First call enables `tracemalloc.start(25)`; subsequent calls return current RSS, traced/peak tracemalloc bytes, and the top-20 file:line allocators. Use to diagnose RSS bloat without restarting.
