...
def brain_value_shipped():
    ...
    # Hardcoded per-source queries — robust to schema drift since each
    # one only needs its own table. The dynamic introspection version
    # this replaced (r32 commit 5f6705d2) returned all-nulls because
    # information_schema lookups across multi-schema setups got tangled.
    def _counts(days):
        return {
            # Brain proposal pipeline — try multiple status spellings.
            "code_fixes": _sql(
                f"SELECT COUNT(*) FROM brain_proposed_fixes "
                f"WHERE COALESCE(applied_at, created_at) >= "
                f"      NOW() - INTERVAL '{days} days' "
                f"AND status IN ('shipped','approved','applied','merged')"
            ),
            # Autopilot actions — any action counts as activity.
            "autopilot_actions": _sql(
                f"SELECT COUNT(*) FROM brain_autopilot_actions "
                f"WHERE COALESCE(completed_at, started_at, created_at) >= "
                f"      NOW() - INTERVAL '{days} days'"
            ),
            # Press releases generated by the auto-writer.
            "press_releases": _sql(
                f"SELECT COUNT(*) FROM auto_press_releases "
                f"WHERE generated_at >= NOW() - INTERVAL '{days} days'"
            ),
            # LinkedIn posts that actually went out.
            "linkedin_posts": _sql(
                f"SELECT COUNT(*) FROM auto_press_releases "
                f"WHERE linkedin_sent_at IS NOT NULL "
                f"  AND linkedin_sent_at >= NOW() - INTERVAL '{days} days'"
            ),
            # Journalist pitches sent — try both possible table names.
            "outreach_pitches": (
                _sql(f"SELECT COUNT(*) FROM media_outreach_log "
                     f"WHERE sent_at >= NOW() - INTERVAL '{days} days'")
                or _sql(f"SELECT COUNT(*) FROM dchub_outreach_log "
                        f"WHERE attempted_at >= NOW() - INTERVAL '{days} days'")
                or 0
            ),
            # Pages newly discovered + healed.
            "facilities_discovered": _sql(
                f"SELECT COUNT(*) FROM discovered_facilities "
                f"WHERE COALESCE(last_seen_at, first_seen_at) >= "
                f"      NOW() - INTERVAL '{days} days'"
            ),
        }

    shipped_7d  = _counts(7)
    shipped_30d = _counts(30)
