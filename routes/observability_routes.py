"""Phase 43 — observability blueprint with click tracking on /snapshot.

Phases 38-42 tried registering a separate /api/v1/observability/conversion-track
endpoint. Push went through every time, file parsed cleanly, but the route
never appeared at runtime. Five separate attempts. Other endpoints in the
same blueprint work fine.

This version stops fighting. Click tracking is a branch INSIDE the existing
/snapshot handler. Funnel rollup is a branch INSIDE /route-audit. Both
endpoints already work, so adding query-param branches inherits that.
  ...
  POST /api/v1/observability/snapshot?event=click     — record upgrade-URL click (NEW BRANCH)
...
# ──────────────────────────────────────────────────────────────────────────
#  /route-audit  (with funnel branch)
# ──────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────
#  /snapshot  (with click branch — supports GET for img-pixel use too)
# ──────────────────────────────────────────────────────────────────────────
...
def snapshot():
    """Snapshot metrics. Branches:
       ...
       (no param, POST)                   → record current metric values
    """
    event = (request.args.get('event') or '').lower()
    if event == 'click':
...
# ──────────────────────────────────────────────────────────────────────────
#  Other endpoints (unchanged from Phase 40)
# ──────────────────────────────────────────────────────────────────────────
