...
def _score_to_100(raw):
    """WRI baseline-water-stress raw (0–5) → 0–100 (100 = most stressed)."""
    r = _num(raw)
    if r is None:
        return None
    r = max(0.0, min(5.0, r))
    return round(r / 5.0 * 100.0, 1)
...
def _extract_rows(payload: str) -> list[dict]:
    ...
    def _consume(props: dict):
        ...
        if not st:
            return
        raw = (props.get("bws_score") if props.get("bws_score") is not None
               else props.get("bws_raw") if props.get("bws_raw") is not None
               else props.get("score"))
        s100 = _score_to_100(raw)
        cat = props.get("bws_cat") or props.get("bws_label") or props.get("category")
        if s100 is None and cat is None:
            return
        rows.append({"state": st, "score100": s100, "raw": _num(raw),
                     "category": (str(cat)[:64] if cat is not None else None)})
...
def run_ingest(dry_run: bool = True) -> dict:
    ...
    if dry_run:
        return {"ok": True, "dry_run": True, "parsed_states": len(rows),
                "preview_most_stressed": preview,
                "note": "dry run — 0 rows written. Verify AZ/NV/CA read MORE stressed than "
                        "IL/OH/MI, then re-POST with ?dry=0.", "wrote": 0}
    c = _conn()
    if c is None:
