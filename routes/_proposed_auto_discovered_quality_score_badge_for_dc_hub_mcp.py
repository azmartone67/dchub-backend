...
Both Smithery and Glama display a quality_score (0-100) and auto-discovered tool list on every server listing; Glama adds a security audit grade and Dockerfile build status. DC Hub is listed on 38-tool registries but exposes no equivalent trust signal, so an agent or developer browsing registries has no quantitative reason to pick us over breadth-only competitors. Smallest shippable version: publish a single /api/v1/mcp/quality endpoint that computes a transparent score from data we already have (48 tools live, sentinel 87 A/12 B page health, uptime) and render a badge SVG we can embed in registry listings and our connect pages. Differentiator: our score is grounded in real operational data (sentinel) competitors can't see. Success = badge present on ≥3 registries and referral lift from registry pages.
