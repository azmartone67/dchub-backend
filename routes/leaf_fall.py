...
def sweep():
    """Compute vitality for every freshness_checks surface; mark low for pruning."""
    _ensure()
    threshold = float(request.args.get("threshold", "10"))
    swept = []
    with _conn() as c, c.cursor() as cur:
        cur.execute("""SELECT surface FROM freshness_checks""")
        surfaces = [r[0] for r in cur.fetchall()]
        for surface in surfaces:
            cur.execute("""SELECT COALESCE(SUM(impressions),0), COALESCE(SUM(engagements),0)
                           FROM page_variants WHERE surface=%s""", (surface,))
            views, clicks = cur.fetchone()
            cur.execute("""SELECT MAX(created_at) FROM page_variant_events
                           WHERE variant_id IN (SELECT id FROM page_variants WHERE surface=%s)
                           AND event='engagement'""", (surface,))
            le = cur.fetchone()[0]
            vit = compute_vitality(int(views or 0), int(clicks or 0), le)
            status = 'alive'
            reason = None
            if vit < threshold and views == 0:
                status = 'wilting'; reason = f"no engagement; vitality {vit}"
            cur.execute("""
                INSERT INTO page_health (surface, views_30d, clicks_30d, last_engagement_at,
            ...
            """, (surface, int(views or 0), int(clicks or 0), le, vit, status, reason))
