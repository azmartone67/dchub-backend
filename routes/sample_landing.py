...
def _safe_pull_monthly(audience: str = "default"):
    try:
        ...
        d = _compute_report()
        return attach_narrative(d, kind="monthly", audience=audience)
    except Exception as e:
        logger.warning(f"_safe_pull_monthly({audience}) failed: {e}")
        return {}


def _safe_pull_quarterly(audience: str = "default"):
    try:
        from routes.report_narrative import attach_narrative
        from routes.comprehensive_report import _gather
        d = _gather(quarter_window=True)
        return attach_narrative(d, kind="quarterly", audience=audience)
    except Exception as e:
        logger.warning(f"_safe_pull_quarterly({audience}) failed: {e}")
        return {}
...
# r42e (2026-05-25): audience-specific framing. Same data, different
# voice + landing-page header — so each audience sees themselves first.
_AUDIENCE_PAGE = {
    "default": {
        "h1":     "The live alternative to CBRE / JLL H2 reports — sample it now.",
        "eyebrow": "DC Hub · Live sample · CC-BY-4.0",
        "lede":   "DC Hub is the live data layer underneath the data-center industry. Every number is machine-readable, refreshed daily, and CC-BY-4.0-licensed — free for journalists, partners, and AI agents. The narratives below are auto-generated by Claude from the underlying structured data.",
        "blurb":  "Generated by claude-haiku-4-5 from the live monthly report. Cached 1 hour. Drop-in CBRE-style analyst voice on live data.",
        "audience_links": "",
    },
    "journalist": {
        "h1":     "For journalists — quote it, cite it, link it. Free.",
        "eyebrow": "DC Hub · Press surface · CC-BY-4.0",
        "lede":   "Live data-center market intelligence in WSJ/Bloomberg lede voice. Quote any sentence with attribution to DC Hub (CC-BY-4.0 — no NDA, no embargo, no license fee). Daily refresh, 285 markets, 13,000+ deals tracked. Pre-written quotables in /reports/monthly.md if you want to skip the curl.",
    ...
    "pe": {
        ...
        "lede":   "Live capital-flow intelligence for data-center deal-committee memos. M&A velocity, hyperscaler $1B+ events, DCPI verdicts across 285 markets — refreshed daily, framed in returns-and-basis voice. Drop into your deal book; cite freely.",
    ...
    "agent": {
        "h1":     "For AI agents — machine-readable, MCP-native, citation-friendly.",
        "eyebrow": "DC Hub · Agent surface · CC-BY-4.0",
        "lede":   "DC Hub is built for AI-agent consumption. 27 MCP tools, /llms.txt discovery, JSON + Markdown + structured narrative — all under CC-BY-4.0 so your agent can cite. The narratives below are optimized for clean fact-extraction.",
        "blurb":  "Tuned for LLM readers: named entities surfaced, parseable phrasing, MW/ISO/company names explicit.",
        "audience_links": '<a class="share" href="https://dchub.cloud/sample/journalist">For journalists →</a> <a class="share" href="https://dchub.cloud/sample/pe">For PE desks →</a> <a class="share" href="https://dchub.cloud/sample">Default view →</a>',
    },
}
...
def sample():
    return _render_sample(audience="default")


@sample_landing_bp.route("/sample/journalist", methods=["GET"], strict_slashes=False)
def sample_journalist():
    return _render_sample(audience="journalist")


@sample_landing_bp.route("/sample/pe", methods=["GET"], strict_slashes=False)
def sample_pe():
    return _render_sample(audience="pe")


@sample_landing_bp.route("/sample/agent", methods=["GET"], strict_slashes=False)
def sample_agent():
...
<head>
  ...
  <link rel="canonical" href="https://dchub.cloud/sample{('/' + audience) if audience != 'default' else ''}">
  <meta property="og:title" content="DC Hub — {page["h1"]}">
  <meta property="og:description" content="{facilities:,} facilities · {total_gw:.1f} GW · {deals_count} deals this month · CC-BY-4.0 · auto-narrative by Claude.">
  <meta property="og:type" content="website">
  <meta property="og:url" content="https://dchub.cloud/sample{('/' + audience) if audience != 'default' else ''}">
  <style>
    :root {{
      --ink: #0a0e1a;
      --paper: #f8fafc;
...
<div class="wrap">
  ...
  <div class="eyebrow">{page["eyebrow"]} · {m_gen or _dt.date.today().isoformat()}</div>
  <h1>{page["h1"]}</h1>
  <p class="lede">{page["lede"]} {facilities:,} facilities · {total_gw:.1f} GW · {deals_count} deals this month.</p>
  <div class="actions" style="margin-bottom:32px">{page["audience_links"]}</div>
  ...
  <p style="color:#94a3b8;font-size:14px;margin-top:0">{page["blurb"]}</p>
