...
def growth():
    ...
    ★★★ THIS ROUTE IS ORIGIN-ONLY. `https://dchub.cloud/data/growth.json` DOES
    NOT REACH IT. That URL is served by a STATIC Cloudflare Pages asset --
    `dchub-frontend/data/growth.json`, regenerated by the frontend's
    `stats-snapshot.yml` cron -- which shares this path and has a COMPLETELY
    DIFFERENT SCHEMA. `_routes.json` `include` lists `/data`, the EXACT path and
    not `/data/*`, so the request is answered by the asset before any worker or
    backend sees it. Reachable only at `dchub-backend-production.up.railway.app`.

        static asset : snapshot_at, current, history_length,
                       facilities{now,delta_7d,delta_30d}, deals{...}, ...
        this route   : as_of, tool_calls_7d, keys_by_tier, top_platforms, source

    ★ BOTH live consumers read the STATIC schema, not this one -- `/growth`
    (`g.snapshot_at`, `g.history_length`) and `/audit`
    (`growth?.iso_endpoints_live?.now`). So nothing is broken today; the hazard
    is READING THIS SOURCE AND INFERRING A PUBLIC SURFACE.

    ★★★ That is not hypothetical: `top_platforms` below made exactly that
    impression in PR #3247, whose description stated this route re-published
    that field to the public at /data/growth.json. It does not. The field is
    real and origin-reachable, but it is not on dchub.cloud.
    Grep finds the code; only a request finds the surface.
    """
