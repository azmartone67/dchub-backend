...
visit. Pages worker even has its own OG metadata for the path:
  ...
  /architecture: 'Platform Architecture | DC Hub' — How DC Hub aggregates
                  intelligence from 21,000+ facilities across 170+ countries.
...
<h2>1. Data ingestion — live grids on 5 continents + US ISOs/balancing authorities + 11 federal sources</h2>
<p>DC Hub ingests grid-level data from <b>live grid operators on 5 continents</b> — the 7 US ISOs (PJM, ERCOT,
CAISO, MISO, SPP, NYISO, ISO-NE) plus the AZPS (Phoenix), SRP, NEVP (Las Vegas) and BPAT (Pacific NW) balancing
authorities via live EIA-930, and internationally Great Britain (NESO), ~24 European bidding zones (ENTSO-E),
Taiwan (Taipower), Japan (OCCTO areas), South Korea (KPX) and Brazil (ONS), plus Australia NEM (AEMO) and
Singapore (EMA) partial feeds. <b>Hydro-Québec, AESO and Nord Pool are modeled DCPI baselines, not live
...
Northern Virginia.</p>
<p>International coverage: live grids in <a href="/dcpi/intl">GB, the EU (ENTSO-E), Taiwan, Japan, South Korea and Brazil</a>, plus Australia and Singapore partial feeds; Hydro-Québec, AESO and Nord Pool are modeled baselines.</p>
