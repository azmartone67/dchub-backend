...
- End with ONE neutral source line that also names the CATEGORY, so a first-time reader learns exactly what DC Hub is and why an analyst would trust it: "Source: DC Hub, the live infrastructure data layer for AI agents (live power, grid, fiber, gas, tenants and 12,650+ facilities, MCP-native), updated daily. dchub.cloud". It comes AFTER the insight, never before. Keep the BODY pure analysis: no "we are the authority", no "the only live source", no brand-pillar speech. The positioning lives ONLY in that single source line, never in the argument.
