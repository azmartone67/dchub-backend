...
def _open_pr(title: str, head: str, body: str,
             draft: bool = False) -> dict | None:
    """Open a PR. `draft` defaults False so every existing caller is unchanged.

    ★★★ DRAFT IS A SAFETY CONTROL HERE, NOT A STYLE CHOICE.
    `.github/workflows/auto-enable-automerge.yml` arms GitHub native auto-merge
    on every NON-DRAFT PR the moment it opens, and it skips drafts on purpose
    — its own comment says arming auto-merge on a draft would turn "a
    propose-only lane into a merging one". So a lane that opens PRs without a
    human in the loop MUST pass draft=True, or its output merges to main and
    deploys on green with nobody having read it.
    """
...
def open_pr_for_finding():
    ...
    # Callers that run without a human in the loop ask for a draft; everything
    # that exists today omits it and keeps its current behaviour.
    want_draft = bool(finding.get("draft"))
    pr = _open_pr(
    ...
    return jsonify(
        ok=True,
        # ★ ECHOED FROM GITHUB'S OWN RESPONSE, never from what we asked for.
        #   An auto lane that depends on draft for its safety must be able to
        #   VERIFY it got one — "we sent draft=True" is not evidence the PR is
        #   a draft, and a silently-ignored field would arm auto-merge.
        draft=bool(pr.get("draft")),
        draft_requested=want_draft,
